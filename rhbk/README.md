...
4. Access the Keycloak route 'example.keycloak.org' and use the 'example-kc-initial-admin' secret for the username and password to login in to the RHBK console
   ~~~
   oc get secret example-kc-initial-admin -o jsonpath='{.data.username}' | base64 --decode && echo # username
   oc get secret example-kc-initial-admin -o jsonpath='{.data.password}' | base64 --decode && echo # password
   ~~~

5. Import the realm "rhdh-realm.json" into Red Hat build of Keycloak